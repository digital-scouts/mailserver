...
router.put('/api/user', async (req, res) => {
  if (!req.query || !req.body.email || !req.body.distributor) {
    res.status(400)
      .send('email or distributor not defined in body');
    return;
  }

  const dis = await Distributor.findOne({ mailPrefix: req.body.distributor as string })
    .exec();
  ...
  if (!dis || !user) {
    res.status(400)
      .send('Distributor or User did not exist');
    return;
  }
...
router.get('/api/distributors', async (req, res) => {
  res.json(await Distributor.find()
    .exec())
    .status(200);
});
...
router.put('/api/distributor', async (req, res) => {
  if (!req.body || !req.body.id || !req.body.distributor) {
    res.status(400)
      .send('You need to provide an id and a distributor object');
    return;
  }
  ...
  if (!distributor) {
    res.status(400)
      .send('No values to change');
    return;
  }

  res.json(await Distributor.findByIdAndUpdate(req.body.id, distributor)
    .exec())
    .status(205);
});
...
router.delete('/api/distributor', async (req, res) => {
  ...
  res.json(await Distributor.findByIdAndDelete(req.body.id)
    .exec())
    .status(200);
});
...
router.post('/api/distributor', async (req, res) => {
  ...
  if (!name) {
    res.status(400)
      .send('Name not defined');
    return;
  }
  if (!description) {
    res.status(400)
      .send('Description not defined');
    return;
  }
  if (!mailPrefix) {
    res.status(400)
      .send('Mail Prefix not defined');
    return;
  }
  ...
  try {
    res.json(await distributor.save())
      .status(201)
      .send();
  } catch (e) {
    res.status(400)
      .send('could not save in DB');
  }
});
